...
func (pt *processorTemplate) Process(input data.BlockData) (data.BlockData, error) {
	...
	output := input
	output.Payset = make([]types.SignedTxnInBlock, 0)
	//for _, txn := range input.Payset {
	//	if txn.Txn.Type == types.ApplicationCallTx && txn.Txn.ApplicationID == 002541853 {
	//		pt.log.Infof("%v: %v", txn, txn.Txn.Type)
	//		output.Payset = append(output.Payset, txn)
	//	}
	//}
